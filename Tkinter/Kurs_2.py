...
class Point:
    ...
    def from_homogeneous(matrix):
        ...
        # Убедимся, что matrix имеет нужную форму, если это не так.
        if matrix.shape == (3,): # Если это одномерный массив (вектор-строка)
            if matrix[2] != 0:
                return Point(matrix[0] / matrix[2], matrix[1] / matrix[2])
            else:
                return Point(matrix[0], matrix[1])
        elif matrix.shape == (1, 3): # Если это массив 1x3
            if matrix[0, 2] != 0:
                return Point(matrix[0, 0] / matrix[0, 2], matrix[0, 1] / matrix[0, 2])
            else:
                return Point(matrix[0, 0], matrix[0, 1])
        else:
            # Обработка других неожиданных форм
            raise ValueError("Неожиданная форма матрицы для преобразования из однородных координат")
...
class GraphicObject:
    ...
    def apply_transform(self, transform_matrix):
        ...
        for p in self.points:
            ...
            new_points_homogeneous.append(Point.from_homogeneous(transformed_hom_coords)) # Убрал reshape(1, 3)
...
class GraphicEditor:
    ...
    def choose_outline_color(self):
        ...
        if color_code:
            ...
            if self.selected_object:
                # Только для объектов, которые поддерживают outline
                item_type = self.canvas.type(self.selected_object.id)
    ...
    def choose_fill_color(self):
        ...
        if color_code:
            ...
            if self.selected_object and hasattr(self.selected_object, 'fill_color'):
                # Только для объектов, которые поддерживают fill (заливку)
                item_type = self.canvas.type(self.selected_object.id)
                if item_type in ("rectangle", "oval", "polygon"): # Line doesn't have fill
    ...
    def start_drawing(self, primitive_type):
        ...
        self.redraw_all_objects() # Убедимся, что все выделения сняты и маркеры убраны
    ...
    def on_canvas_click(self, event):
        if self.drawing_primitive:
            ...
            # Если в режиме перемещения и есть выбранный объект, начинаем перетаскивание
    ...
    def highlight_selected_object(self):
        ...
        for obj in self.objects:
            if obj.id: # Убедимся, что объект уже отрисован
                # Получаем тип объекта Tkinter для корректной настройки
                item_type = self.canvas.type(obj.id)
                if item_type in ("line", "rectangle", "oval", "polygon"):
                    # Для объектов, поддерживающих outline
                    self.canvas.itemconfig(obj.id, outline=obj.color, width=2)
                elif item_type == "text":
                    # Если это текстовый объект, можете настроить цвет заливки (текста)
                    self.canvas.itemconfig(obj.id, fill=obj.color)
                # Для image нет outline, можно проигнорировать или добавить свою логику выделения
        ...
        if self.selected_object:
            ...
            if item_type in ("line", "rectangle", "oval", "polygon"):
                ...
                self.canvas.itemconfig(self.selected_object.id, fill="red") # Пример выделения текста
            # Для image можно было бы нарисовать поверх него пунктирный прямоугольник
            # или другую форму выделения, если это нужно.
            
            # Отображаем центр фигуры, если он определен
