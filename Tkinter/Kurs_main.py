...
class GraphicEditor:
    ...
    def start_drawing(self, primitive_type):
        ...
        if primitive_type == "bezier":
            messagebox.showinfo("Кривая Безье", "Кликните 3 или 4 раза для задания контрольных точек кривой Безье. (3 точки для квадратичной, 4 для кубической)") # Инструкции для Безье
    ...
    def on_canvas_click(self, event):
        # Обработчик события клика мыши по холсту
        if self.drawing_primitive: # Если активен режим рисования примитива
            ...
            if self.drawing_primitive == "line":
                ...
                # Для кривой Безье требуется 3 (квадратичная) или 4 (кубическая) контрольные точки
                if len(self.temp_points) == 3 or len(self.temp_points) == 4:
                    bezier_curve = BezierCurve(self.temp_points, self.current_color) # Создание объекта BezierCurve
                    self.objects.append(bezier_curve) # Добавление кривой в список объектов
                    self.drawing_primitive = None # Сброс режима рисования
                    self.canvas.config(cursor="arrow") # Изменение курсора
                    self.selected_object = bezier_curve # Выбор только что созданной кривой
                    self.redraw_all_objects() # Перерисовка
                    ...
                    self.redraw_all_objects() # Перерисовка для очистки временных маркеров
            ...
            self.draw_transform_marker(self.transform_center.x, self.transform_center.y, "#FF0000") # Отрисовка маркера центра
            angle = simpledialog.askfloat("Поворот", "Введите угол поворота (градусы):") # Запрос угла у пользователя
            if angle is not None and self.selected_object: # Если угол введен и объект выбран
    ...
    def on_canvas_drag(self, event):
        ...
        if self.dragging_object and self.selected_object and self.current_transformation_mode == "translation":
            ...
            if len(self.temp_points) == 1: # Если есть только одна контрольная точка
                self.bresenham_line(self.temp_points[0], Point(event.x, event.y), "#AAAAAA", width=1) # Отрисовка временной линии от первой точки до текущего положения курсора
            elif len(self.temp_points) >= 2: # Если есть две или более контрольных точек
        ...
        for obj in reversed(self.objects): # Итерация в обратном порядке (сверху вниз)
            if isinstance(obj, (Cross, Flag)):
                ...
                for cp in obj.control_points:
                    if math.sqrt((x - cp.x)**2 + (y - cp.y)**2) < 10:
                        return obj
    ...
    def redraw_all_objects(self):
        ...
        if self.current_transformation_mode == "rotation_around_point" and hasattr(self, 'transform_center'):
            self.draw_transform_marker(self.transform_center.x, self.transform_center.y, "#FF0000") # Отрисовка маркера центра вращения
        ...
        if self.selected_object and self.selected_object.center and self.current_transformation_mode != "select_tmo_objects":
            self.draw_transform_marker(self.selected_object.center.x, self.selected_object.center.y, "#00FF00") # Отрисовка центра выбранного объекта зеленым
    ...
    def draw_transform_marker(self, x, y, color_hex):
        # Рисование маркера центра трансформации (сочетание крестика и круга)
        marker_size = 5 # Размер маркера

        # Круг (множество пикселей по окружности)
        for angle in np.linspace(0, 2 * np.pi, 30): # Итерация по 30 точкам на окружности
            mx = x + marker_size * math.cos(angle) # X-координата точки на окружности
            my = y + marker_size * math.sin(angle) # Y-координата точки на окружности
            self.put_pixel(mx, my, color_hex) # Установка пикселя

        # Крест (две линии)
        self.bresenham_line(Point(x - marker_size * 2, y), Point(x + marker_size * 2, y), color_hex) # Горизонтальная линия креста
        self.bresenham_line(Point(x, y - marker_size * 2), Point(x, y + marker_size * 2), color_hex) # Вертикальная линия креста
    ...
    def start_rotation_around_point(self):
        ...
        messagebox.showinfo("Поворот (Rc)", "Кликните на холсте, чтобы задать центр поворота.") # Инструкция
    ...
    def start_mirror_vertical_line(self):
        ...
        messagebox.showinfo("Зеркальное отражение (MV)", "Кликните на холсте, чтобы задать вертикальную линию отражения.") # Инструкция
