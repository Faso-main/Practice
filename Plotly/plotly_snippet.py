...
wbs_items_timed = [
    ...
    ('4', 'Оценка, тест-завершение проекта', 1, 0, '3.3'), # Фаза 4 начинается после завершения 3.3 (основная разработка ИИ-компонентов)
...
# Сортируем задачи по ID. ID в формате '1', '1.1', '1.1.1' обеспечивают правильный порядок зависимостей при сортировке строк.
...
for item_id, item_text, item_level, duration, dependency_id in wbs_items_timed:
    ...
    # Находим дату старта на основе зависимости
    if dependency_id:
        if dependency_id in task_finish_dates:
            ...
            # Эта ситуация все еще может произойти, если зависимость указана неверно или на несуществующую задачу
            print(f"Warning: Dependency ID '{dependency_id}' not found or processed yet for task '{item_id}'. Starting after project start date.")
            start_date = project_start_date # Запасной вариант
...
fig = ff.create_gantt(gantt_data,
                      ...
                      bar_width=0.6,
                      # show_arrows=False, # Этот параметр вызывает ошибку и удален
                     )
